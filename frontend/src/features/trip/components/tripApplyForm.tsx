...
export default function TripApplyForm({
  ...
  // const [approvalSteps, setApprovalSteps] = useState<ApprovalStepWithApprover[]>([]);
  ...
  //   await approveTripPreApply(initialData.id, comment);
  //   // 成功後の処理（例：再フェッチ or 画面遷移）
  // };

  // const handleReject = async (comment: string) => {
  //   if (!initialData?.id) return;
  //   await rejectTripPreApply(initialData.id, comment);
  //   // 成功後の処理（例：再フェッチ or 画面遷移）
  // };
